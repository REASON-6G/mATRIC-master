from sqlalchemy import Column, String, JSON, ForeignKey, Boolean, Table, DateTime
...
class AgentCommand(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True, nullable=False)
    agent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id"), nullable=False)  # Foreign key to Agents table
    command = Column(String, nullable=False)
    timestamp = Column(DateTime, default=datetime.utcnow)  # Automatically sets the timestamp when the command is created
