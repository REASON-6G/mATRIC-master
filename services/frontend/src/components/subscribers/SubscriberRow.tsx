...
export default function SubscriberRow({ subscriber, onViewToken, onRefresh }: SubscriberRowProps) {
  ...
  const saveField = async (field: SubscriberField, value: string) => {
    if (!idForActions) {
    ...
    setSaving(true);
    try {
      const payload: Partial<Pick<Subscriber, SubscriberField>> = { [field]: value };
