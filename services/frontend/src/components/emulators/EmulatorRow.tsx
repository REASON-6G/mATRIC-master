...
export default function EmulatorRow({ emulator, refresh }: EmulatorRowProps) {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const res = publisherId
          ? await api.get<Topic[]>(`/api/publishers/${publisherId}/topics`)
          : await api.get<Topic[]>("/api/topics/");
  ...
  const saveField = async (
    field: "name" | "topic_id" | "interval" | "msg_schema" | "publisher_id",
    value: string | number | unknown
  ) => {
    if (field === "name") setName(value as string);
    if (field === "topic_id") setTopicId(value as string);
    if (field === "interval") setInterval(value as number);
    if (field === "publisher_id") setPublisherId(value as string | null);
    ...
    <>
      ...
      {editingSchema && (
        <EditSchemaModal
          ...
          onSave={(schema) => saveField("msg_schema", schema)}
        />
      )}
