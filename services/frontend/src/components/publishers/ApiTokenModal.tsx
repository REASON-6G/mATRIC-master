...
export default function ApiTokenModal({ publisherId, onClose }: ApiTokenModalProps) {
  ...
  const fetchToken = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get<{ api_token: string }>(`/api/publishers/${publisherId}/token`);
  ...
  const regenerateToken = async () => {
    setLoading(true);
    try {
      const res = await api.post<{ api_token: string }>(`/api/publishers/${publisherId}/token/regenerate`);
      setToken(res.data.api_token);
