...
export default function PublisherRow({ publisher, onViewToken, onRefresh }: PublisherRowProps) {
  ...
  const saveField = async (field: PublisherField, value: string | number) => {
    setSaving(true);

    try {
      const payload: Partial<Publisher & { location?: { type: "Point"; coordinates: [number, number] } }> = {};
      ...
      if (field === "latitude" || field === "longitude") {
        ...
        if (latVal !== "" && lonVal !== "") {
          ...
          const lonNum = typeof lonVal === "string" ? parseFloat(lonVal) : lonVal;
        ...
        payload[field] = value as string; // only name/org/country/city
      }
