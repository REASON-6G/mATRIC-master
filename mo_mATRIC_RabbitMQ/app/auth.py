...
class AuthManager:
    ...
    def get_authenticated_user_or_app(self, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db_manager)) -> TokenData:
        ...
        try:
            ...
            payload = jwt.decode(token, settings.jwt_secret_key, algorithms=[settings.jwt_algorithm])
            print("or_app payload: ", payload)
            scope: str = payload.get("scope")
            print("or_app scope: ", scope)
            sub: str = payload.get("sub")
            print("or_app sub: ", sub)
...
def get_authenticated_user_or_app(token: str = Depends(oauth2_scheme), db: DatabaseManager = Depends(get_db_manager)) -> TokenData:
    return auth_manager.get_authenticated_user_or_app(token, db)